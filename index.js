...
💬 Or just chat with AI!`, { 
  ...
  bot.command('image', async (ctx) => {
    ...
    if (!prompt) {
      return sendMsg(chatId, 'Usage: /image <prompt>\n\nExample: /image Cricket stadium with crowd cheering');
    }
    ...
    try {
      ...
      if (imageUrl) {
        ...
        sendMsg(chatId, '❌ Failed to generate image. Please try again.');
      }
    } catch (error) {
      console.log('Image gen error:', error.message);
      sendMsg(chatId, '❌ Image generation requires credits. Please use the web app at https://sports247-2.onrender.com for full features.');
    }
  });

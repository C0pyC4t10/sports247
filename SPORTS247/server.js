...
// Load brand
try {
  const brandPath = path.join(__dirname, 'brand', 'brand.js');
  ...
  BRAND = { 
    ...
    negativePrompts: ["blurry, low quality", "deformed, ugly", "watermark, text"],
    trendingModifiers: ["trending on artstation", "award winning", "cinematic"],
    qualityModifiers: "professional, 4k, ultra detailed",
    stylePresets: { cinematic: "cinematic lighting, film grain" },
    defaultEnhancement: "professional sports photography"
  };
}
...
app.post('/telegraf', express.json(), async (req, res) => {
  try {
    if (!telegrafBot) {
      // Try to load bot
      try {
        ...
        const botFactory = botModuleImport.initBot;
        const botObj = botFactory();
        telegrafBot = botObj.bot;
        console.log('✅ Bot loaded for webhook');
      } catch (e) {
